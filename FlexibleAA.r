#Flexible AA Study
FlexAA<-function(data,lookback=c(4,4,4),wt=c(1,.5,.5),n.top=.5,cash.col=1){
    ...
    library(xts)
    library(TTR)
    ac.ret<-mom_ac_hist_ret(data,lookback.mom)
    ac.ret.rank<-mom_ac_ret_rank(data,lookback.mom)
    ac.sd.rank<-vol_ac_sd_rank(data,lookback.vol) #ranks of each asset class
    ac.cor.rank<-cor_ac_cor_rank(data,lookback.cor)
