#Flexible AA Study
faa.old<-function(data,lookback=c(4,4,4),wt=c(1,.5,.5),n.top=.5,cash.col=1,data.type="prices"){
...
faa_orig<-function(data,lookback=c(4,4,4),wt=c(1,.5,.5),n.top=.5,cash.col=1,data.type="prices"){
    # original approach (from the paper) that weights relative momentum, volatility and correlation
    # data xts of daily prices
    # cash.col columns which is the cash substitute when abs mom is negative
    lookback.mom<-lookback[1] ; lookback.vol<-lookback[2] ; lookback.cor<-lookback[3] # months to lookback to calc values
    wt.mom<-wt[1] ;    wt.vol<-wt[2] ;    wt.cor<-wt[3] #wts of ranks in scoreing
